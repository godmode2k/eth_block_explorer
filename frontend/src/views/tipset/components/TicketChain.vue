...
*/
    ...
    async hash(v) {
      ...
      if (this.hashList.includes(v)) {
        console.log( "watch::hash() hashList.includes()" );
        //this.drawChart();
      } else {
        console.log( "watch::hash() else{}" );
/*
        //for parent hash click
        let data = await getBlockByCid({
          cid: this.$route.query.hash
...
/*
      ...
      this.jumpSafeHeight = height;
*/
      console.log( "mounted(): get latest block number" );
      const data = await getLatestBlockNumber();
      const res_json = JSON.parse(data.result);
      const height = res_json.block_number;
      console.log( "height = " + height );
      currentHeight = height;
      this.jumpSafeHeight = height;
    }
    //this.getTipset(currentHeight);
    this.getBlockByHeight(currentHeight);
    /*
    chart.on("click", e => {
      if (e.data.originData) {
        this.$emit("hash-change", e.data.originData.cid);
  ...
  methods: {
    ...
    console.log( "getBlockByHeigh(): height = " + height );
      if (height == 0) {
        return;
      }
      if (height < 15) {
      ...
      try {
        ...
        */
        //blocks = blocks.map(item => {
        blocks = JSON.parse(data.result).map(item => {
          //const { size, cid, reward, block_header, msg_cids } = item;
        ...
        //  return item.hash;
        //});
        const hashList = JSON.parse(data.result).map(item => {
          return item.block_hash;
        });
        this.hashList = hashList;
        this.startHeight = height;
        //this.tipsets = Object.freeze(reverse);
        this.tipsets = Object.freeze(blocks);
        this.$emit("get-blocks", blocks);
        //this.drawChart();
      } catch (e) {
        this.loading = false;
