...
Filename:   eth_txns_fetcher_main.go
...
1. Build:
	$ go build eth_txns_fetcher_main.go
    or
...
func get_blocks_all() {
    ...
    // if duplicated, update(replace) it...
    {
        ...
        fmt.Println( "block height from DB (str) = ", block_height_from_db )
        block_height_from_db_uint64, _ = strconv.ParseUint( block_height_from_db, 10, 64 )
        fmt.Println( "block height from DB = ", block_height_from_db_uint64 )
    }
    ...
    for {
        ...
        fmt.Println( "[block height] = ", block_height_old_uint64, "/", block_num_end_uint64 )
        ...
        if ( count_repeat > 1 ) {
            fmt.Println( "skip... count = ", count_repeat )
            continue
        }
